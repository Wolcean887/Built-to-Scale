...
@onready var player_animation = $Player_sprite
...
func _physics_process(delta):
	...
	if direction:
		if is_on_floor():
			if can_dash:
				if direction < 0:
					player_animation.play("Walk_Dash")
				else:
					player_animation.play("Walk_Dash_F")
			else:
		...
		if is_on_floor():
			if can_dash:
				if direction < 0:
					player_animation.play("Idle_Dash")
					$Player_sprite.flip_h = false
				else:
					player_animation.play("Idle_Dash_F")
					$Player_sprite.flip_h = true
			else:
	...
	if not is_on_floor():
		if can_dash:
			if direction < 0:
				player_animation.play("Jump_Dash")
			else:
				player_animation.play("Jump_Dash_F")
				$Player_sprite.flip_h = true
		else:
