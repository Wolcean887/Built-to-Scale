...
var platform_music = preload("res://audio/GMTK Jam24 Platform Theme 2 (no reverb tail).mp3")
...
func play_platform_music():
	if !$AudioStreamPlayer.is_playing() or !$AudioStreamPlayer.stream.resource_path == "res://audio/GMTK Jam24 Platform Theme 2 (no reverb tail).mp3":
